...
[検索のための準備：コーパスの読み込み](overview_sejong#検索のための準備：コーパスの読み込み)を参考に，書きことばあるいは話しことばのコーパスを読み込みます．平文コーパスの場合，自分で作成したテキストファイルを読み込むことも可能です．その場合，テキストファイルはUTF-16LEエンコーディング（BOM付き）で作成しておく必要があります．
